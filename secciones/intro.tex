...
\cite{} & From sensor networks without cameras to smart cameras in 3D environments & Review \\
...
[27] & Solutions for indoor positioning are proposed based on novel classifications, sensor data, kinds of detected items, and localization technique. & Survey \\
...
En este sentido, en el trabajo presente se propone abordar dicha brecha mediante \begin{enumerate}
    \item Captura y anotación de un conjunto de datos propio, recolectado con la cámara y resolución reales del sistema objetivo
    \item Entrenamiento supervisado de una arquitectura recurrente híbrida diseñada para explotar dependencias temporales y aprender una transformación hacia una escala métrica ajustada a nuestras condiciones de captura.
\end{enumerate}
...
La estimación inicial de distancias, obtenida a partir de los parámetros intrínsecos de la cámara y de características observables de los sujetos en el entorno, permite establecer una aproximación métrica que sirve como base para el entrenamiento supervisado de una arquitectura recurrente híbrida. 

% Quitar la segunda parte del párrafo de la hipótesis ya que no es parte de la misma : JESÚS GARCÍA RAMIREZ
% Se plantea que, al incorporar dependencias temporales presentes en las secuencias de video y aprovechar la variabilidad de altura entre los distintos sujetos, el modelo puede aprender una representación más generalizable y coherente de la escala métrica, mejorando la precisión de las estimaciones de distancia en escenas dinámicas capturadas con cámara monocular.
