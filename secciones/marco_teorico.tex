...
\begin{align}
    \text{Predict:}\quad & \mathbf{x}_{k|k-1} = F \mathbf{x}_{k-1|k-1},\qquad 
    P_{k|k-1} = F P_{k-1|k-1} F^\top + Q, \\
    \text{Update:}\quad & K_k = P_{k|k-1} H^\top (H P_{k|k-1} H^\top + R)^{-1},\\
...
\begin{enumerate}
  \item \textbf{Componente de movimiento (Mahalanobis):} mide la discrepancia entre la detección \(\mathbf{d}_j\) y la predicción del filtro de Kalman para el track \(i\), teniendo en cuenta la incertidumbre del estado. Su forma es
  \[
    d_{\mathrm{M}}^2(i,j) = (\mathbf{d}_j - H\mathbf{x}_{i,k|k-1})^\top S_i^{-1}(\mathbf{d}_j - H\mathbf{x}_{i,k|k-1}),
  \]
  donde \(S_i = H P_{i,k|k-1} H^\top + R\) es la covarianza de la predicción proyectada al espacio de medición. En la práctica se aplica un \emph{gating} estadístico: asociaciones cuyo \(d_{\mathrm{M}}^2\) excede un umbral (por ejemplo, cuantiles de la distribución \(\chi^2\)) se consideran imposibles y se descartan antes de la asignación \cite{mahalanobis1936generalised}.
  
  \item \textbf{Componente de apariencia (similaridad):} para cada detección se extrae un \emph{embedding} visual \(f_j\in\mathbb{R}^D\) mediante una red CNN preentrenada para re-identificación (ReID). La similitud entre el embedding de la detección y el embedding representativo del track \(i\) se evalúa típicamente mediante la distancia coseno:
  \[
    d_{\cos}(i,j) = 1 - \frac{f_i^\top f_j}{\lVert f_i\rVert \lVert f_j \rVert}.
  \]
  DeepSORT mantiene un histórico corto de embeddings por track y usa la mínima o la media de distancias sobre ese banco para robustecer la comparación frente a variaciones de vista y oclusión .
\end{enumerate}

Estos dos términos se combinan en un costo total ponderado
\begin{equation}
    c(i,j) = \lambda \, d_{\mathrm{M}}^2(i,j) + (1-\lambda)\, d_{\cos}(i,j),
\end{equation}
...
Dada la matriz de costos \(C=[c(i,j)]\), la correspondencia entre \(M\) tracks y \(N\) detecciones se obtiene resolviendo un problema de asignación con costo mínimo. En la práctica DeepSORT emplea el algoritmo húngaro para obtener la asignación \(X=[x_{ij}]\) que minimiza \(\sum_{i,j} c(i,j) x_{ij}\) sujeto a restricciones de unicidad \cite{kuhn1955hungarian}. Para mejorar la robustez en presencia de tracks con distintas edades, DeepSORT aplica una \emph{matching cascade} que procesa primero tracks recientemente actualizados y deja para etapas posteriores los tracks más antiguos, reduciendo así los cambios de identidad (ID switches) en ventanas temporales con oclusiones parciales \cite{wojke2017simple}.
